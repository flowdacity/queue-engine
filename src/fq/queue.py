...
class FQ(object):
    ...
    def _load_lua_scripts(self):
        ...
        with open(os.path.join(lua_script_path, "enqueue.lua"), "r") as enqueue_file:
            self._lua_enqueue_script = enqueue_file.read()
            self._lua_enqueue = self._r.register_script(self._lua_enqueue_script)

        with open(os.path.join(lua_script_path, "dequeue.lua"), "r") as dequeue_file:
            self._lua_dequeue_script = dequeue_file.read()
            self._lua_dequeue = self._r.register_script(self._lua_dequeue_script)

        with open(os.path.join(lua_script_path, "finish.lua"), "r") as finish_file:
            self._lua_finish_script = finish_file.read()
            self._lua_finish = self._r.register_script(self._lua_finish_script)

        with open(os.path.join(lua_script_path, "interval.lua"), "r") as interval_file:
            self._lua_interval_script = interval_file.read()
            self._lua_interval = self._r.register_script(self._lua_interval_script)

        with open(os.path.join(lua_script_path, "requeue.lua"), "r") as requeue_file:
            self._lua_requeue_script = requeue_file.read()
            self._lua_requeue = self._r.register_script(self._lua_requeue_script)

        with open(os.path.join(lua_script_path, "metrics.lua"), "r") as metrics_file:
            self._lua_metrics_script = metrics_file.read()
            self._lua_metrics = self._r.register_script(self._lua_metrics_script)
    ...
    async def enqueue(
        ...
        await self._lua_enqueue(keys=keys, args=args)
    ...
    async def dequeue(self, queue_type="default"):
        ...
        dequeue_response = await self._lua_dequeue(keys=keys, args=args)
    ...
    async def finish(self, job_id, queue_id, queue_type="default"):
        ...
        finish_response = await self._lua_finish(keys=keys, args=args)
    ...
    async def interval(self, interval, queue_id, queue_type="default"):
        ...
        args = [interval]
        interval_response = await self._lua_interval(keys=keys, args=args)
    ...
    async def requeue(self):
        ...
        active_queue_type_list = await self._r.smembers(
        ...
        for queue_type in active_queue_type_list:
            ...
            args = [timestamp]
            job_discard_list = await self._lua_requeue(keys=keys, args=args)
    ...
    async def metrics(self, queue_type=None, queue_id=None):
        ...
        * list of queue ids for each queue type.
        ...
        if not queue_type and not queue_id:
            ...
            active_queue_types = await self._r.smembers(
                "%s:active:queue_type" % self._key_prefix
            )
            ready_queue_types = await self._r.smembers(
            ...
            args = [timestamp]
            enqueue_details, dequeue_details = await self._lua_metrics(
                keys=keys, args=args
            )
            enqueue_counts = {}
            dequeue_counts = {}
